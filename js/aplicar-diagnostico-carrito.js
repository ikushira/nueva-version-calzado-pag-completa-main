// aplicar-diagnostico-carrito.js
// Script para añadir la herramienta de diagnóstico a todas las páginas HTML
const fs = require('fs');
...
htmlFiles.forEach(htmlFile => {
    ...
    // Verificar si el archivo ya incluye el diagnóstico de carrito
    if (content.includes('diagnostico-carrito-tarjetas.js')) {
        console.log(`${htmlFile}: Ya incluye el diagnóstico de carrito, no se requieren cambios`);
        return;
    }
    
    // Buscar donde insertar el diagnóstico (después del último script relacionado con carrito)
    ...
    for (const script of carritoScripts) {
        if (content.includes(script) && !updated) {
            ...
            updated = true;
            console.log(`✅ ${htmlFile}: Diagnóstico de carrito añadido después de ${script}`);
            break;
        }
    }
    ...
    if (!updated) {
        ...
        console.log(`✅ ${htmlFile}: Diagnóstico de carrito añadido antes del cierre del body`);
        updated = true;
    }
