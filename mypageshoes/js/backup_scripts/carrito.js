...
// Función global para agregar un producto una sola vez (necesaria para el fix de tarjetas)
...
function obtenerDatosProducto(boton) {
  ...
  let talla = "";
  // Buscar específicamente en el contenedor del producto actual
  const tallasContainer = contenedor.querySelector('.producto-tallas, .tallas-list');
  
  if (tallasContainer) {
    const botonTallaSeleccionado = tallasContainer.querySelector('.talla-btn.selected, .talla-btn.active');
    if (botonTallaSeleccionado) {
      talla = botonTallaSeleccionado.textContent.trim();
    }
  } else {
    console.warn("No se encontró contenedor de tallas en el producto");
  }
...
function enlazarBotonesAgregarAlCarrito() {
  ...
  // En lugar de verificar constantemente, verificamos solo al cambiar la página
  if (window.carritoObservador) {
    window.carritoObservador.disconnect();
  }
  
  // Usar MutationObserver para detectar nuevos botones cuando se modifica el DOM
